...
class BetaEmulPred:
    ...
    def __init__(self, bnorm_file, netsurfp_results, out_dir):
        self.out_dir = out_dir
        # Save the normalization values in a dataframe
        self.norm_df = pd.read_csv(bnorm_file, index_col=0)
    ...
    def save_beta(self):
        ...
        txt_file(self._adjusted_predictions, os.path.join(self.out_dir, 'b_results.txt'))
...
if __name__ == '__main__':
    ...
    # python Beta_emulsifier.py -n ../netsurfp2_results.txt

    # Parse it in
    parser = argparse.ArgumentParser()
    parser.add_argument('-n', dest='netsurfp_results', default='', help='txt file with netsurfp results')
    parser.add_argument('-v', dest='norm_vals', default=os.path.join('..', 'NormalizationValues', 'b_norm.csv'),
    ...
    bclass = BetaEmulPred(args.norm_vals, args.netsurfp_results, args.out_dir)
    bclass.peptide_cutoffs(nr_names=args.nr_names, score=args.lower_score)
    bclass.save_beta()
